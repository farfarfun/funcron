...
from funcron.center.pages.decorated import login_required
from funcron.center.pages.main.core import blue_print
...
@blue_print.route("/cron_list", methods=["GET", "POST"])
...
def cron_list():
    ...
    page_data = CronInfos.cron_list(page=page, task_name=task_name, page_size=20)
    if "page" in keyword:
        del keyword["page"]
...
def job_log_all_list():
    ...
    page_data = (
        JobLog.query.join(CronInfos, CronInfos.id == JobLog.cron_info_id)
        .filter(*filter_arr)
...
def job_log_clear():
    ...
    db.session.delete(job_logs)
    db.session.commit()
    return web_api_return(code=0, msg="清空成功")
